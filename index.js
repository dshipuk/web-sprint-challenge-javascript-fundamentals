...
//🚀🚀🚀 ⬇️ 📝 Explanation ⬇️ 📝 🚀🚀🚀: 
/* the nested function can access internal because its in the same scope */
...
const zooAnimals = [
  ...
  */
  ...
  function animalNames(arr){
    ...
    zooAnimals.forEach((info) => {
      temp.push(`name: ${info.animal_name}, scientific: ${info.scientific_name}`)
    })
    return temp
  }
  ...
  */
  ...
  function lowerCaseNames(arr){
    ...
    let temp = [];
    arr.map((data) => {
      temp.push(data.animal_name.toLowerCase())
    })
    return temp
  }
  ...
  */
  ...
  function lowPopulationAnimals(arr){
    return arr.filter(info => info.population < 5)
  }
  ...
  */
  ...
  function USApop(arr){
    return arr.reduce((total, info) => total + info.population, 0)
  }
